...
def shrink_poly(vertices, coef=0.3):
    ...
    if cal_distance(x1, y1, x2, y2) + cal_distance(x3, y3, x4, y4) > \
        ...
        offset = 0  # two longer edges are (x1y1-x2y2) & (x3y3-x4y4) #width
    else:
...
def get_boxes(score, geo, score_thresh=0.9, nms_thresh=0.2):
    ...
    Input:
        score       : score map from run <numpy.ndarray, (1,row,col)>
        geo         : geo map from run <numpy.ndarray, (5,row,col)>
...
class Trainset(data.Dataset):
    def __init__(self, img_path, gt_path, scale=0.25, length=512):
        ...
        self.img_files = [os.path.join(img_path, img_file) for img_file in sorted(os.listdir(img_path)) if
                          ".txt" not in img_file]
        self.gt_files = [os.path.join(gt_path, gt_file) for gt_file in sorted(os.listdir(gt_path)) if ".txt" in gt_file]
        self.scale = scale
        self.length = length
    ...
    def __getitem__(self, index):
        ...
        # print_error("img :" + str(img.size))
        # print_error("img file : " + str(self.img_files[index]))
...
class ICDARDatasetV1(IDataset):
    def __init__(self,
        ...
        self._train_set = Trainset(img_path=data_in_dir+"/train/",
                                   gt_path=data_in_dir+"/train/",
                                   scale=0.25,
                                   length=512)

    def _get_num_train_samples(self):
        img_path = self._data_in_dir + "/train/"
        return len([os.path.join(img_path, img_file) for img_file in sorted(os.listdir(img_path)) if
                    ".txt" not in img_file])
    ...
    def get_torch_train_data_loaders(self):
        loader = data.DataLoader(self.get_torch_train_dataset(),
