...
def main(fname, out_name=None):
    img = Image.open(fname)
    im = img.convert('RGB')
    im = np.array(im.copy())
    blur = reduce_noise_raw(im)
    edges = auto_canny(blur.copy())
    debordered = reduce_noise_edges(edges.copy())
    dilation, rects, num_tries = find_components(debordered, 16)
    final_rect = find_final_crop(dilation, rects)
...
#
#     raw_images = glob(os.path.join(config.IMAGE_ROOT_DIR + '/*'))
...
class docToTextReceiptLocalisation(pluginApplication):

    def plugin_inputs(self):
        # Custom location according to need
        self.source_folder = config.DOC2TEXT_DIR
...
if __name__ == '__main__':
    t = docToTextReceiptLocalisation()
    t.plugin_inputs()
    print('--' * 55)
    t.bulk_run()
