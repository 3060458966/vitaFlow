...
def argparser():
    ...
    ap.add_argument('-a', '--asciify', default=log_missing_ascii_mappings, action='store_true',
...
def convert_directory(directory, options):
    ...
    for fn in sorted(files):
        ...
        if not os.path.exists("csv"):
            os.makedirs("csv")
        # print(len(conll_data))
        conll_data = conll_data.replace("\n\n", "\n")
        conll_data = conll_data.replace("~", "^")
        if "" in conll_data:
            print("found", os.path.basename(fn))

        with open(os.path.join("csv", os.path.splitext(os.path.basename(fn))[0] + ".csv"), "w+") as out_f:
            out_f.write("0\t1\n")
            out_f.write(conll_data.encode('utf-8'))
