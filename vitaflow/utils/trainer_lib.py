...
"""Library for training. See vf_trainer.py."""
...
from vitaflow.utils import devices
from vitaflow.utils import hyperparams_lib as hparams_lib
from vitaflow.utils import metrics_hook
...
def create_run_config(model_name,
  ...
  if not use_tpu:
    ...
    use_distribution_strategy = (
        ...
        vf_model.VfModel.has_symmetric_shards(model_name) and
...
def create_estimator(model_name,
  ...
  model_fn = vf_model.VfModel.make_estimator_model_fn(
...
def create_experiment(
  ...
  train_hooks += vf_model.VfModel.get_train_hooks(model_name, hook_context)
  eval_hooks += vf_model.VfModel.get_eval_hooks(model_name, hook_context)
